...
def get_training_input_contextual(filenames, filename_emo, params):
    ...
    with tf.device("/cpu:0"):
        ...
        sample_dataset = tf.data.TextLineDataset(sample_file)
...
#        code.interact(local=locals())
        ...
        # Split string
        dataset = dataset.map(
            lambda src, tgt, emo, ctx_dia_src, ctx_dia_tgt, ctx_sty_src, ctx_sty_tgt, ctx_lan_src, ctx_lan_tgt, pos_dia_src, pos_dia_tgt, pos_sty_src, pos_sty_tgt, pos_lan_src, pos_lan_tgt, sample: (
...
#        code.interact(local=locals())
        dataset = dataset.padded_batch(
            params.eval_batch_size,
            {
                ...
                "sample_length": [],
                "references": (tf.Dimension(None),) * (len(inputs) - 15)
            },
            {
                "source": params.pad,
                "source_length": 0,
                ...
                "sample": params.pad,
                "sample_length": 0,
                "references": (params.pad,) * (len(inputs) - 15)
            }
        )
