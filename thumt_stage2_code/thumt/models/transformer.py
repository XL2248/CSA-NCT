...
#        context_sty_src = transformer_context(context_input, ctx_attn_bias, params)
        context_sty_src, _ = transformer_encoder(context_input, ctx_attn_bias, params, get_first_layer)
...
#        context_sty_tgt = transformer_context(context_input, ctx_attn_bias, params)
        context_sty_tgt, _ = transformer_encoder(context_input, ctx_attn_bias, params, get_first_layer)
...
#        context_dia_tgt = transformer_context(context_input, ctx_attn_bias, params)
    ...
    if mode == "infer":
        dia_mask = top_mask
        return encoder_output, context_dia_src, context_dia_tgt, context_sty_src, context_sty_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb,turn_sty_src_pos_emb, turn_sty_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sa_tgt, dia_mask
...
#    return encoder_output, context_dia_src, context_dia_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sa_tgt, top_mask
    return encoder_output, context_dia_src, context_dia_tgt, context_sty_src, context_sty_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb,turn_sty_src_pos_emb, turn_sty_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sa_tgt, top_mask
...
def decoding_graph(features, state, mode, params):
    ...
    dec_attn_bias = layers.attention.attention_bias(tf.shape(targets)[1],
                                                    "causal", dtype=dtype)
    if params.use_coherence or params.use_speaker:
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
...
#                                              params, state=state["decoder"])
    ...
    if params.use_speaker:
        translated = tf.reduce_sum(decoder_output * t_mask, -2) / tf.reduce_sum(t_mask, -2)
        with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
            speaker_weight = tf.get_variable("speaker_weights",
                                            [2, hidden_size * 2],
                                            initializer=initializer, trainable=True)
        binary_1 = tf.matmul(tf.concat([translated, context_speaker_src], -1), speaker_weight, False, True)
        binary_0 = tf.matmul(tf.concat([translated, context_speaker_tgt], -1), speaker_weight, False, True)
        sp1_ce = losses.smoothed_softmax_cross_entropy_with_logits(logits=binary_1, labels=tf.ones([tf.shape(translated)[0], 1]), smoothing=params.label_smoothing, normalize=True)
        sp2_ce = losses.smoothed_softmax_cross_entropy_with_logits(logits=binary_0, labels=tf.zeros([tf.shape(translated)[0], 1]), smoothing=params.label_smoothing, normalize=True)
...
#    print(logits)
    ...
    if params.use_bowloss:
        ...
        #code.interact(local=locals())
    '''
    return ce_loss, kl_loss, avg_bow_loss, sp_loss, coh_loss, clus_loss, clm_loss


def model_graph(features, mode, params):
    encoder_output, context_dia_src, context_dia_tgt, context_sty_src, context_sty_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb, turn_sty_src_pos_emb, turn_sty_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sample_output, dia_mask = encoding_graph(features, mode, params)
    state = {
...
class Transformer(NMTModel):
    ...
    def get_inference_func(self):
        def encoding_fn(features, params=None):
            ...
            with tf.variable_scope(self._scope):
                encoder_output, context_dia_src, context_dia_tgt, context_sty_src, context_sty_tgt, context_source, turn_dia_src_pos_emb, turn_dia_tgt_pos_emb, turn_sty_src_pos_emb, turn_sty_tgt_pos_emb, turn_ctx_src_pos_emb, first_layer_output, sample_output, dia_mask = encoding_graph(features, "infer", params)
