...
#        all_loss = [ce_loss, tf.to_float(kl_weights) * kl_loss, tf.to_float(kl_weights) * bow_loss, tf.to_float(kl_weights) * coh_loss, tf.to_float(kl_weights) * clus_loss]
#        total_loss = ce_loss + tf.to_float(kl_weights) * (kl_loss + bow_loss + sp_loss + coh_loss + clus_loss + clm_loss)
        total_loss = ce_loss + tf.to_float(kl_weights) * (kl_loss + bow_loss + coh_loss) + tf.to_float(kl_weights2) * (sp_loss)
#        total_loss = ce_loss + kl_loss + bow_loss + sp_loss + coh_loss + clus_loss + clm_loss
#        total_loss = [ce_loss, kl_loss, bow_loss, clm_loss, sp_loss, coh_loss, clus_loss]
        # Optimization
...
#            files = [params.validation] + [params.dev_dialog_src_context] + [position_file_src_dia] + [params.dev_dialog_tgt_context] + [position_file_tgt_dia] + [position_file_ctx_src] + [params.dev_sample] + [params.dev_context_source] + list(params.references)
            files = [params.validation] + [params.dev_dialog_src_context] + [position_file_src_dia] + [params.dev_dialog_tgt_context] + [position_file_tgt_dia] + [position_file_ctx_src] + [params.dev_sample] + [params.dev_context_source] + [params.dev_style_src_context] + [position_file_src_sty] + [params.dev_style_tgt_context] + [position_file_tgt_sty] + list(params.references)
