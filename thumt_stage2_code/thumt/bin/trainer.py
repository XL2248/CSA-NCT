...
def main(args, kl_weights=1.0, kl_weights2=1.0):
    ...
    with tf.Graph().as_default():
        ...
        for item in s_losses:
            sharded_loss, kl_loss, bow_loss, sp_loss, coh_loss = item[0], item[1], item[2], item[3], item[4]
        ...
        sp_loss = tf.add_n(sp_losses) / len(sp_losses)
        coh_loss = tf.add_n(coh_losses) / len(coh_losses)
...
#        alpha = kl_weights
 ...
 #       if params.use_bowloss:
        total_loss = ce_loss + tf.to_float(kl_weights) * (kl_loss + bow_loss) + tf.to_float(kl_weights2) * (sp_loss + coh_loss)
